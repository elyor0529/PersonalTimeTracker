...
func parseCmdFlagsSetUpOrm() {
	var database = 
		flag.String("database", "postgres", "the database to use (currently supported: postgres)")
	var databaseUserName = 
		flag.String("dbuser", "postgres", "database's user")
	var databasePassword = 
		flag.String("dbpassword", "123", "database's password")

	if value, ok := os.LookupEnv("PGPASSWORD"); ok {
		databasePassword = &value
	}
	var offlineDatabaseFile = 
		flag.String("sqliteDbPath", ".\\data.db", "sqlite database file's path")
	var onlineDatabaseName = 
		flag.String("dbname", "sandbox", "database name in the online datastore")
	var hostName = 
		flag.String("ipaddr", "127.0.0.1", "database server's ip address")
	var hostPort = 
		flag.String("port", "5432", "database server port")
